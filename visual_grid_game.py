...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, custom_walls=None):
        ...
        # Place toxic traps only on safe, empty cells so they never overlap walls, food, or the start position.
        available_cells = [
            (x, y)
        ...
        trap_count = min(random.randint(3, 5), len(available_cells))
        self.toxic_traps = set(random.sample(available_cells, trap_count))
    ...
    def _cell_ahead(self) -> tuple[int, int]:
        dx, dy = self._direction_delta(self.agent_direction)
