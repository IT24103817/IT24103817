...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, custom_walls=None):
        ...
        # Step 2.1: Declare toxic_traps set avoiding (0,0), walls, and food
    ...
    def get_percept(self) -> dict:
        return {
            ...
            # Step 2.2: Sensor key added for toxic traps
    ...
    def execute_action(self, action: str):
        ...
        # Step 2.3: Decrement score by 15 points if stepping on a toxic trap
...
class GridGameGUI:
    ...
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=2, walls=None):
        ...
        # GreedyGridAgent instance එකක් සාදා ගැනීම
    ...
    def draw_grid(self):
        ...
        # Step 2.3: Render custom purple shapes for toxic traps on canvas
        for tx, ty in self.env.toxic_traps:
            offset = self.cell_size * 0.2
            x1 = tx * self.cell_size + offset
            y1 = (self.env.height - 1 - ty) * self.cell_size + offset
            self.canvas.create_rectangle(x1, y1, x1 + self.cell_size * 0.6, y1 + self.cell_size * 0.6, fill="#8b5cf6",
                                         outline="#6d28d9")
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                # 1. Percept eka environment eken aragen agent ta denawa
                percept = self.env.get_percept()
                
                # 2. Agent smart action eka thiranaya karanawa
                action = self.agent.sense_and_act(percept)
                
                # 3. Action eka execution karanawa
