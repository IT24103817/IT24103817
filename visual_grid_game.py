...
class GridGameGUI:
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=2, walls=None):
        self.root = root
        self.root.title("IT3012 - IA Search Agent")
        ...
        self.label = tk.Label(root, text="Score: 0 | Steps: 0 | Food Left: 0", font=("Arial", 14))
        ...
        def step():
            if not self.env.is_done():
                ...
                if self.env.collision:
                    end_text = f"💥 Collision! Final Score: {self.env.score}"
                elif len(self.env.food_positions) == 0:
                    end_text = f"🎉 YOU WIN! Final Score: {self.env.score}"
                else:
                    end_text = f"🏁 Game Over! Final Score: {self.env.score}"
