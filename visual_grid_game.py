...
class VisualGridHuntGame:
    """A flexible Pacman-style grid environment with support for configurable opponents, traps, and larger scales."""
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, num_traps=3, custom_walls=None):
        ...
        # Generate toxic traps safely avoiding (0,0), walls, and food
    ...
    def execute_action(self, action: str):
        ...
        # Check food collection (+20 points)
        if tuple_pos in self.food_positions:
            self.food_positions.remove(tuple_pos)
            self.score += 20

        # Check intersection with toxic traps (-15 points penalty)
        if tuple_pos in self.toxic_traps:
            self.score -= 15

        # Opponent movement and collision logic
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        # Draw Toxic Traps (Custom Purple Triangles)
        for tx, ty in self.env.toxic_traps:
            x1 = tx * self.cell_size
            y1 = (self.env.height - 1 - ty) * self.cell_size

            points = [
                x1 + self.cell_size / 2, y1 + self.cell_size * 0.2,   # Top apex
                x1 + self.cell_size * 0.2, y1 + self.cell_size * 0.8, # Bottom left
                x1 + self.cell_size * 0.8, y1 + self.cell_size * 0.8  # Bottom right
            ]
