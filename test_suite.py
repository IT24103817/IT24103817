...
#this is the test suite for the agent classes
...
# this is the test suite for the search agent
# also test heuristic functions
class TestPractical3_SearchAgent(unittest.TestCase):
    ...
    def test_dfs_path_correctness(self):
        """Test 5: DFS must find a valid path to the goal, and fail if unreachable."""
        grid_size = (4, 4)
        start_pos = (0, 0)
        goal_pos = (3, 3)
        walls = [(1, 0), (2, 0), (0, 2), (1, 2), (2, 2)]
        ...
        self.assertIsNotNone(path, "DFS returned None instead of a path.")
        self.assertIsInstance(path, list, "DFS should return a list of actions (strings).")

        # Verify the path actually leads to the goal from start without hitting walls
        ...
        for action in path:
            ...
            # Check bounds
            self.assertTrue(0 <= current[0] < grid_size[0] and 0 <= current[1] < grid_size[1],
                            f"DFS path went out of bounds at {current}")
            # Check walls
            self.assertNotIn(current, walls, f"DFS path hit a wall at {current}")

        self.assertEqual(current, tuple(goal_pos), "DFS path did not reach the goal.")

        # Test unreachable goal for DFS
        unreachable_walls = [(1, 2), (2, 1), (1, 1)]
        path_unreachable = self.search_agent.dfs_search((0, 0), (2, 2), unreachable_walls, (3, 3))
        is_empty_or_none = (path_unreachable is None) or (len(path_unreachable) == 0)
        self.assertTrue(is_empty_or_none, "DFS should return None or [] when the goal is unreachable.")

    def test_ucs_shortest_path(self):
        """Test 6: UCS must find the optimal (shortest) path in a static maze, and fail if unreachable."""
        grid_size = (4, 4)
        start_pos = (0, 0)
        goal_pos = (3, 3)
        walls = [(1, 0), (2, 0), (0, 2), (1, 2), (2, 2)]
        ...
        # Test unreachable goal for UCS
        unreachable_walls = [(1, 2), (2, 1), (1, 1)]
        path_unreachable = self.search_agent.ucs_search((0, 0), (2, 2), unreachable_walls, (3, 3))
        is_empty_or_none = (path_unreachable is None) or (len(path_unreachable) == 0)
