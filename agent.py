...
# it is a problem solving agent that plans paths using bfs, dfs, ucs, and A*.
# it is also a offline planning agent that plans paths in a static environment.
class SearchAgent:
    """A search agent that plans paths using BFS, DFS, UCS, and A*."""
    ...
    def sense_and_act(self, percept: dict) -> str:
        if not self.plan:
            ...
            sorted_food = sorted(all_food, key=lambda f: self.manhattan_distance(f, start_pos))
            ...
            for food in sorted_food:
                if self.active_algo == 'BFS':
                    ...
                    path = self.astar_search(start_pos, food, walls, grid_size)
...
# this is the ucs search algorithm implementation for shortest path.
    ...
    def astar_search(self, start_pos, goal_pos, walls, grid_size):
        ...
        # Priority queue elements: (f_cost, g_cost, current_pos, path_taken)
        pq = []
        reached_states = set()
        g_cost = 0
        ...
        while pq:
            _, current_g_cost, current_pos, path_taken = heapq.heappop(pq)
            if current_pos == goal:
                return path_taken
            ...
            for action, (dx, dy) in [('Up', (0, 1)), ('Down', (0, -1)), ('Left', (-1, 0)), ('Right', (1, 0))]:
                ...
                if not (0 <= nxt[0] < width and 0 <= nxt[1] < height):
                    continue
                if nxt in walls or nxt in reached_states:
                    continue

                g_new = current_g_cost + 1
                h_new = self.manhattan_distance(nxt, goal)
                f_new = g_new + h_new
                heapq.heappush(pq, (f_new, g_new, nxt, path_taken + [action]))
